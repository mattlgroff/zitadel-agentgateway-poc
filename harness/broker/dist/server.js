...
app.get("/.well-known/oauth-authorization-server", async (context) => {
    ...
    if (!response.ok)
        return context.text(await response.text(), response.status);
...
app.post("/register", async (context) => {
    ...
    if (typeof body.client_name !== "string" || !body.client_name.trim())
        return context.json({ error: "invalid_client_metadata", error_description: "client_name is required" }, 400);
    if (!Array.isArray(redirects) || redirects.length === 0 || !redirects.every((uri) => typeof uri === "string" && validRedirect(uri)))
        return context.json({ error: "invalid_redirect_uri" }, 400);
    if (!body.application_type || !["web", "native"].includes(String(body.application_type)))
        return context.json({ error: "invalid_client_metadata", error_description: "application_type is required" }, 400);
    if (body.token_endpoint_auth_method !== "none")
        return context.json({ error: "invalid_client_metadata", error_description: "token_endpoint_auth_method must be none" }, 400);
    ...
    if (!response.ok)
        return context.json(result, response.status);
